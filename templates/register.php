...
<body>
<h1 class="visually-hidden"><?= $contentTitle ?></h1>
    ...
    <div class="container container--with-sidebar">
        <header class="main-header">
            ...
            <div class="main-header__side">
                <a class="main-header__side-item button button--transparent" href="/user-auth.php">Войти</a>
            </div>
        </header>
        ...
        <div class="content">
            <section class="content__side">
                ...
                <a class="button button--transparent content__side-button" href="/user-auth.php">Войти</a>
            </section>

            <main class="content__main">
                <h2 class="content__main-heading">Регистрация аккаунта</h2>

                <form class="form" action="/user-register.php" method="post" autocomplete="off">
                    <div class="form__row">
                        <label class="form__label" for="email">E-mail <sup>*</sup></label>

                        <input class="form__input <?= isset($error['email']) ? ' form__input--error' : '' ?>"
                               type="text" name="email" id="email" value="<?php echo isset($_POST['email'])? strip_tags($_POST['email']): '' ?>" placeholder="Введите e-mail">

                        <?= isset($error['email']) ? '<p class="form__message">' . $error['email'] . '</p>' : '' ?>
                    </div>

                    <div class="form__row">
                        <label class="form__label" for="password">Пароль <sup>*</sup></label>

                        <input class="form__input  <?= isset($error['password']) ? ' form__input--error' : '' ?>" type="password" name="password" id="password" value="" placeholder="Введите пароль">
                        <?= isset($error['password']) ? '<p class="form__message">' . $error['password'] . '</p>' : '' ?>
                    </div>

                    <div class="form__row">
                        <label class="form__label" for="name">Имя <sup>*</sup></label>

                        <input class="form__input <?= isset($error['name']) ? ' form__input--error' : '' ?>"
                               type="text" name="name" id="name" value="<?php echo isset($_POST['name'])? strip_tags($_POST['name']): '' ?>" placeholder="Введите имя">
                        <?= isset($error['name']) ? '<p class="form__message">' . $error['name'] . '</p>' : '' ?>
                    </div>

                    <div class="form__row form__row--controls">
                        <?= !empty($error) ? '<p class="error-message">Пожалуйста, исправьте ошибки в форме</p>' : ''?>
